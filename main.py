...
def get_data():
    ...
    for i in req_obj['list']:
        v.append([i['dt'], i['dt_txt'].replace(':00:00', 'ч'), i['main'][
            'temp'] - 273.15])  # Выбор необходиммых данных; изменение формата времени, температуры на более подходящий
...
def visualise_data(json_data=''):
    if json_data:
        ...
        scatplot(dates, temps)  # Построим первый грфик

        boxplot(dates, temps)  # Построим второй график


def scatplot(dates, temps):
    ax = pplt.axes()
    pos = ax.get_position()
    pos.y0 = 0.2
    ax.set_position(pos)

    # построим данные на диаграмме рассеяния
    ...
    # Указываем деления и размеры шкал
    pplt.yticks(np.arange(math.floor(min(temps) - 1), math.floor(max(temps) + 1), 1))
    pplt.xticks(np.arange(len(dates)), dates,
                rotation=80)

    pplt.show()  # выводим первый график


def boxplot(dates, temps):
    # Собираем данные на второй график
    bdata = []  # Температура за все рассматриваемые дни
    data = []  # Температура за день
    days = []  # Список рассмотренных дат
    lst = 0  # Фиксирует смену дня на следующий
    for i in range(len(temps)):  # проходимся по списку показаний
        now = dates[i][8:10]  # парсим список показаний, извлекаем номер рассматриваемого в данный момент дня

        if lst != now:  # Проверяем, сменился ли день, или это тот же день что и прошлый, просто другое время
            bdata.append(data)  # Если день новый, сохраняем предыдущие данные как отдельный день
            days.append(str(lst))
            data = []  # и заново начинаем собирать данные уже за этот
            lst = now
        data.append(temps[i])  # в любом случае записываем данные

    days.append(dates[len(dates) - 1][8:10])  # дозакрываем последний день, после которого не настал новый
    bdata.append(data)
